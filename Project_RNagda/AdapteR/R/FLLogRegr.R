...
glm.FLTable<-function(formula,data,rushil="binomial",iter=25,threshold=0.1,...){
	...
	AnalysisID<-as.vector(sqlQuery(data@odbc_connection,paste0("CALL FLLogRegr('",deeptablename,"', 'ObsID', 'VarID', 'Num_Val',",iter,",",threshold,", 'Test',AnalysisID);"))[1,1])
...
residuals.FLLogRegr<-function(object){
	...
	queryscore<-paste0("CALL FLLogRegrScore ('",object@deeptablename,"','ObsID','VarID','Num_Val',NULL,'",object@AnalysisID,"','",scoretable,"','Scoring using model ",object@AnalysisID,"',oAnalysisID);")
...
lrdata.FLLogRegr<-function(object){
		...
		sqlstr2<-paste0("SELECT a.* FROM fzzlLogRegrCoeffs a WHERE a.AnalysisID = '",object@AnalysisID,"' order by COEFFID;")
