...
lu.FLMatrix<-function(object)
{
	...
	# finding limit
	limit<-sqrt((object@nrow)*(object@ncol))
	...
	perm <- new("FLVector", 
		         ...
		         col_name = table@num_val_name, 
		         vector_id_value = max_vector_id_value-1, 
		         size = object@ncol)   ###############################  check this for non-square matrices
